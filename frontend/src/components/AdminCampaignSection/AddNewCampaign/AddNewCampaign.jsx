...
const AddNewCampaign = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "vaccineType") {
      const selectedVaccine = vaccineOptions.find((vaccine) => vaccine.name === value);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isTimeDifferenceValid()) {
      toast.error("Time difference between start and end time should be 1 hour.");
      return;
    }

    try {
      const response = await fetch("http://localhost:3005/timeSlot/createTimeSlot", {
        method: "POST",
        headers: {
