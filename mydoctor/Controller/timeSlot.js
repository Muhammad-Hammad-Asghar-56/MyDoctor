...
class TimeSlotController {
  static createCampaignTimeSlot = async (req, res) => {
    ...
    if (error) {
      return res.status(400).json({ sucess: false, error: error.details[0].message });
    }
  ...
  static getAllTimeSlotToRegiesterNurseT = async (req, res) => {
  ...
  static registerNurse = async (req, res) => {
    ...
    const result = await NurseTimeSlotDBHandler.registerNurseTimeSlot(nurseId, timeSlotId);
    if (result) {
      return res.status(200).json({ success: true, error: "Successfully register nurse in time slot" })
    }
    
    return res.status(200).json({ success: true, error: "Failed to register nurse in time slot" })
  }
  ...
  static unRegisterNurse = async (req, res) => {
    ...
    try{
      const {nurseId,timeSlotId}=req.body;      
      const result = NurseTimeSlotDBHandler.findNurseTimeSlot(nurseId,timeSlotId,req.body);
      if(!result){
        return res.status(400).json({success:false,error:"Cannot find nurse in given slot"});
      }
      
      result = await NurseTimeSlotDBHandler.unRegisterNurseTimeSlot(nurseId, timeSlotId);
      // return res.status(200).json({success:true,message:"Delete Successfully"});
      if(result){
        return res.status(200).json({success:true,message:"Delete Successfully"});
      }
      return res.status(200).json({success:true,message:"Fail to Delete"});
    }
    catch(err){
      return res.status(500).status({success:false,error:"Server Error"})
    }
  }
}
