...
class SchedulePatientDBHandler {
  ...
  static haveClearPreviousVaccineRecord(patientSSN, vaccineId) {
    return new Promise((resolve, reject) => {
      const query = `
        SELECT coalesce(max(round),0) AS round 
        FROM patientschedule ps 
        JOIN timeslot ts ON ps.TimeslotID = ts.TimeslotID 
      ...
      connection.query(query, [patientSSN, vaccineId], (error, results) => {
        ...
        resolve(results[0].round);
      });
    });
  }
  static markVaccination(patientSSN, timeSlotId) {
    let updateQuery = "UPDATE patientschedule SET onHold=0 WHERE TimeslotID =?  and patientSSN=?";
    return new Promise((resolve, reject) => {
      try {
        connection.query(updateQuery, [timeSlotId, patientSSN], (err, result) => {
          if (err) {
            console.error(err);
            reject(err);
            return;
